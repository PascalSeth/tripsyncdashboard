...
// Debug helper to print FormData in browser console
const debugLogFormData = (label: string, url: string, fd: FormData) => {
  ...
  try {
    ...
    for (const [key, value] of (fd as any).entries()) {
      if (value instanceof File) {
        entries.push({ key, type: 'File', name: value.name, size: value.size, mime: value.type })
      } else {
    ...
    console.warn('FormData debug error', e)
  }
}
...
const Products = () => {
  ...
  const fetchStores = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (data.data.stores && data.data.stores.length > 0 && !selectedStoreId) {
  ...
  const fetchProducts = async (page = 1, search = '', category = '', subcategory = '', inStock = '') => {
    ...
    try {
      ...
      const response = await fetch(`/api/stores/${selectedStoreId}/products?${queryParams}`)
      const data = await response.json()
  ...
  const deleteProduct = async (productId: string) => {
    ...
    try {
      const response = await fetch(`/api/stores/${selectedStoreId}/products/${productId}`, {
        method: 'DELETE'
      })
  ...
  const handleCreate = async () => {
    if (!formData.name.trim() || !formData.price || !formData.categoryId || !selectedStoreId) {
    ...
    try {
      ...
      const url = `/api/stores/${selectedStoreId}/products`
      debugLogFormData('Product Create Payload', url, formDataToSend)
      const response = await fetch(url, {
        method: 'POST',
        body: formDataToSend
      })
      ...
      let debugText = ''
      try { debugText = await respClone.text() } catch {}
      console.debug('Create product response', { status: response.status, body: debugText?.slice(0, 1000) })
      const data = await response.json()
  ...
  const handleSaveEdit = async () => {
    if (!formData.name.trim() || !formData.price || !formData.categoryId || !editingId || !selectedStoreId) {
    ...
    try {
      ...
      const url = `/api/stores/${selectedStoreId}/products/${editingId}`
      debugLogFormData('Product Update Payload', url, formDataToSend)
      const response = await fetch(url, {
        method: 'PUT',
        body: formDataToSend
      })
      ...
      let debugText = ''
      try { debugText = await respClone.text() } catch {}
      console.debug('Update product response', { status: response.status, body: debugText?.slice(0, 1000) })
      const data = await response.json()
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4 py-6">
          <div className="flex justify-between items-center">
            ...
            {!isCreating && !editingId && selectedStoreId && (
              <button
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex items-center gap-4">
            ...
            <Select value={selectedStoreId || "select"} onValueChange={(value) => setSelectedStoreId(value === "select" ? "" : value)}>
              <SelectTrigger className="w-64">
                <SelectValue placeholder="Choose a store" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="select">Choose a store</SelectItem>
                {stores.map((store) => (
            ...
            {selectedStore && (
              <div className="text-sm text-gray-600">
                Managing products for: <span className="font-medium">{selectedStore.name}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {selectedStoreId && (
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            <div className="flex flex-wrap gap-4 items-center">
              ...
              <div className="flex items-center gap-2">
                <Select value={filterSubcategory || "all"} onValueChange={(value) => setFilterSubcategory(value === "all" ? "" : value)}>
                  <SelectTrigger className="w-40">
              ...
              <div className="flex items-center gap-2">
                <Select value={filterInStock || "all"} onValueChange={(value) => setFilterInStock(value === "all" ? "" : value)}>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <Select
                  value={formData.categoryId || "select"}
                  onValueChange={(value) => handleInputChange({ target: { name: 'categoryId', value: value === "select" ? "" : value } } as any)}
                  disabled={actionLoading === 'create' || actionLoading === 'edit'}
                >
              ...
              <div>
                ...
                <Select
                  value={formData.subcategoryId || "select"}
                  onValueChange={(value) => setFormData(prev => ({ ...prev, subcategoryId: value === "select" ? "" : value }))}
                  disabled={actionLoading === 'create' || actionLoading === 'edit'}
                >
                  <SelectTrigger className="w-full">
                  ...
                  <SelectContent>
                    <SelectItem value="select">Select subcategory</SelectItem>
                    {subcategories.map(subcategory => (
                      <SelectItem key={subcategory.id} value={subcategory.id}>
                        {subcategory.name}
                      </SelectItem>
        ...
        {loading && selectedStoreId ? (
          ...
          <div className="text-center py-12">
            ...
            <div className="text-gray-500 text-lg mb-2">Select a Store</div>
            <p className="text-gray-400">Choose a store from the dropdown above to manage its products</p>
          </div>
        )}

        {/* Summary Stats */}
        {!loading && selectedStoreId && products.length > 0 && (
